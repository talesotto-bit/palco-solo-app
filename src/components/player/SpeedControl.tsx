...
export function SpeedControl({ compact = false }: SpeedControlProps) {
  ...
  const bpmDisplay = (baseBpm: number) => Math.round(baseBpm * speed)
      ...
      <div className="flex items-center justify-center gap-1.5 md:gap-1 flex-wrap">
        {SPEED_PRESETS.map(p => (
          <button
            ...
            className={cn(
              'h-9 min-w-[40px] md:h-7 md:min-w-[36px] rounded-md text-xs md:text-[10px] font-bold transition-colors',
